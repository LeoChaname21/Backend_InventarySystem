...
public class IncomeServiceImpl implements IncomeService {
    ...
    private Boolean validateSign(String fileName){
        String ruta=Paths.get("C:", "uploads","firmas") + File.separator;
    ...
    private String createJOrden(Income income){
        log.info("=== INICIO GENERACIÓN ORDEN ===");
        log.info("Income ID: {}", income.getId());
        log.info("Estado (autorizado): {}", income.getEstado());
        ...
        log.info("Cantidad de items en incomeDetailWrappers: {}", incomeDetailWrappers.size());
        ...
        parameters.put("REPORT_DIR", Paths.get("C:", "uploads") + File.separator);
        ...
        log.info("Cantidad de items enviados a Jasper: {}", incomeOrder.size());
        ...
        try{
            ...
            if(income.getEstado()){
                parameters.put("REPORT_AUTH",Paths.get("C:", "uploads","firmas" ,income.getUserAuth().getNombre()+".png") + File.separator);
                doc="ordenAuth.jrxml";
            }

            String ruta = Paths.get(Paths.get("C:", "uploads","orders").toString(), incomeId + ".pdf").toString();
            createEmptyPDF(ruta);

            log.info("JRXML usado: {}", doc);
            log.info("Ruta JRXML: {}",
                    Paths.get("C:", "uploads", "report", doc).toString());

            JasperReport report= JasperCompileManager.compileReport(Paths.get(Paths.get("C:", "uploads", "report").toString(),doc).toString());

            log.info("Parámetros enviados a Jasper:");
            parameters.forEach((k, v) -> log.info("  {} = {}", k, v));

            JasperPrint print= JasperFillManager.fillReport(report, parameters, new JREmptyDataSource());

            log.info("Ruta PDF salida: {}", ruta);
